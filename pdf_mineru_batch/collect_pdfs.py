r"""Collect PDFs into one external data folder and rename them sequentially.
...
def sequential_pdf_name(index: int, total_count: int, prefix: str) -> str:
    width = max(3, len(str(total_count)))
    return f"{prefix}{index:0{width}d}.pdf"
...
def cleanup_generated_pdfs(output_dir: Path, name_prefix: str) -> None:
    pattern = re.compile(rf"^{re.escape(name_prefix)}\d+\.pdf$", re.IGNORECASE)
...
def collect_pdfs(
    ...
    if not keep_original_names and output_dir != source_dir:
        cleanup_generated_pdfs(output_dir, name_prefix)

    excluded_dirs = [output_dir]
    if exclude_dirs:
        excluded_dirs.extend(path.resolve() for path in exclude_dirs)
    ...
    pdf_paths = sorted(
        ...
        and not any(is_inside(path, excluded_dir) for excluded_dir in excluded_dirs)
    )

    with manifest_path.open("w", newline="", encoding="utf-8-sig") as manifest_file:
        writer = csv.DictWriter(
            manifest_file,
            fieldnames=["index", "source_path", "copied_path", "file_name", "batch_name"],
        )
        writer.writeheader()
        ...
        for index, source_pdf in enumerate(pdf_paths, start=1):
            target_name = (
                ...
                else sequential_pdf_name(index, len(pdf_paths), name_prefix)
            )
            target_pdf = unique_target_path(output_dir, target_name)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Prefix for renamed copied PDFs. Default: {DEFAULT_NAME_PREFIX}",
    )
    parser.add_argument(
        "--keep-original-names",
        action="store_true",
        help="Keep original PDF file names instead of renaming to pdf001.pdf, pdf002.pdf, ...",
    )
